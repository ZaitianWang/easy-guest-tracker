...
import uhf.detailwith.RwDataDetailWith;
import uhf.structures.InventoryArea;
import uhf.structures.InventoryData;
import uhf.structures.OnInventoryListener;
import uhf.structures.OnRwListener;
import uhf.structures.RwData;
import uhf.structures.SerialNumber;
import uhf.structures.SoftVersion;
	...
	public static int initial(String port) {
		...
		if (Linkage.getInstance().openCom(port, 115200L) == 0) {
			...
			if (Linkage.getInstance().getModuleSerialNumber(serialNumber) == 0) {
				String serialNo = uhf.utils.StringUtils.byteToHexString(serialNumber.snData, serialNumber.snLen);
